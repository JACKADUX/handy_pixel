...
static func get_palette_image(colors:PackedColorArray, rect_size := Vector2(10,10), x_count :int = 20) -> Image:
	if not colors:
		return
	var y_count = ceil(colors.size()/float(x_count))
	var image = Image.create_empty(rect_size.x*x_count, rect_size.y*y_count, false, Image.FORMAT_RGBA8)
	for index in colors.size():
		var color = colors[index]
		var pos = Vector2(index%x_count*rect_size.x, index/x_count*rect_size.y)
		image.fill_rect(Rect2(pos, rect_size), color)
	return image
